...
for element in f_train.readlines():
    ...
    # 图片的路径
    root = os.path.join('H:\\datasets\\Postdam\\Color\\', element + '.png')
    # 图片保存的新路径
    save_path = os.path.join('H:\\datasets\\VOCPotsdam\\Train\\mask_vis\\', element + '.png')
    # 读取图片
    img = cv.imread(root)
    # 写入图片
...
for element in f_val.readlines():
    element = str(element).replace("\n", "")
    root = os.path.join('H:\\datasets\\Postdam\\Color\\', element + '.png')
    save_path = os.path.join('H:\\datasets\\VOCPotsdam\\Val\\mask_vis\\', element + '.png')
    img = cv.imread(root)
    save_img = cv.imwrite(save_path, img)