...
file_names = glob.glob(r'C:/Remote sensing semantic segmentation/ISPRSVaihingen/Val/RGB/*.tif')
print(file_names)
...
for name in file_flags:
    image_rgb = cv2.imread('C:/Remote sensing semantic segmentation/ISPRSVaihingen/Val/RGB/top_mosaic_09cm_area' + name + '.tif')
    image_label = cv2.imread('C:/Remote sensing semantic segmentation/ISPRSVaihingen/Val/Color/top_mosaic_09cm_area' + name + '.tif')  # top_potsdam_2_10_label.tif
    ...
    for i in range(min_i // size_of_split):  # [512, 512, 3]
        for j in range(min_j // size_of_split):
            ...
            cv2.imwrite('C:/Remote sensing semantic segmentation/ISPRSVaihingen/Val/image/' + 'v' + str(num) + '.jpg', img_rgb)
            cv2.imwrite('C:/Remote sensing semantic segmentation/ISPRSVaihingen/Val/mask_vis/' + 'v' + str(num) + '.png', img_label)
